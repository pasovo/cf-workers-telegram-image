...
function AppContent({ isAuthed, setIsAuthed }: { isAuthed: boolean; setIsAuthed: (v: boolean) => void }) {
  // 顶层声明所有用到的 useState/useEffect/useRef/useCallback/useMemo
  // 登录相关
  ...
  const handleBatchMove = async (targetFolder: string) => {
    ...
    try {
      ...
      if (data.status === 'success') {
        ...
        fetchHistory(search, tagFilter, filenameFilter, page, false);
        setSelected([]);
      } else {
  ...
  const handleRenameFolder = async (oldName: string, newName: string) => {
    ...
    try {
      ...
      if (data.status === 'success') {
        ...
        setRenameFolder('');
        // 刷新图库
        if (tab === 'gallery') fetchHistory(search, tagFilter, filenameFilter, 1, false);
      } else {
  ...
  const fetchFolders = async () => {
    try {
      ...
      const data = await res.json();
      if (data.status === 'success') {
        setFolderList(['/', ...data.folders.filter((f: string) => f !== '/')]);
      }
    } catch {}
  };
  // 新增统一的 fetchGalleryOverview 方法
  ...
  return (
    <div className="w-full min-h-screen bg-[#10151b]">
      ...
      {/* Banner区块已移除 */}
      {/* 查找并移除 container、max-w-4xl、px-2、sm:px-4、py-8 等外层包裹 */}
      {/* 只保留 w-full min-h-screen 作为最外层，内容卡片直接渲染在其下 */}
